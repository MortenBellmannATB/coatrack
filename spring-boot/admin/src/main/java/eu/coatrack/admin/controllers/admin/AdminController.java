...
public class AdminController {
    ...
    public ModelAndView gettingStartedWizardForConsumer() {
        ...
        ServiceApi gettingStartedTestService = serviceApiService.getTestServiceForConsumerWizard(valueConfig);
    ...
    public ModelAndView serviceWizard(ServiceWizardForm wizard) {
        ...
        if (authenticatedUser != null) {
            ...
            Proxy proxy = proxyService.create(authenticatedUser, serviceApi, valueConfig.getProxyServerDefaultPort());
            ApiKey apiKey = apiKeyService.create(authenticatedUser, serviceApi);
    ...
    public String refreshApiKeys(@PathVariable("whichFragmentToLoad") String whichFragmentToLoad, Model model) {
        ServiceApi gettingStartedTestService = serviceApiService.getTestServiceForConsumerWizard(valueConfig);
        List<ApiKey> apiKeysForTestService = apiKeyService.findByLoggedInAPIConsumerAndServiceId(gettingStartedTestService.getId());
        ...
        List<String> proxiesUrlList = proxyService.customSearchForAllProxiesForGivenServiceApiId(newApiKey.getServiceApi().getId());
        ...
        List<String> defaultProxyList = !proxiesUrlList.isEmpty() ? Collections.singletonList(proxiesUrlList.get(0)) : new ArrayList<>();
    ...
    public void updateNotificationStatusOnGatewayHealthMonitor(@RequestParam String proxyId, @RequestParam boolean isMonitoringEnabled) {
        proxyService.updateNotificationStatusOnGatewayHealthMonitor(proxyId, isMonitoringEnabled);
    }
